...
titulo = Label(L1, text="CADASTRO DO VENDEDOR")
titulo["font"] = ("Arial", "20", "bold")
titulo.pack()

LnomeVend = Label(L2,text="NOME DO VENDEDOR", font=fontePadrao)
LnomeVend.pack(side=LEFT)
EnomeVend = Entry(L2,width=40,font=fontePadrao)
EnomeVend.pack(side=LEFT)
...
def Func_carregaTV(): # Lê os dados do banco e envia para treeview
    ...
    try:
        ...
        cursor.execute("SELECT  idvend, nomevend FROM Tbvendedor ORDER BY idvend ASC")
        ...
        for row in rows:
            ...
            tv.insert("", 'end', text=idvend, values=(idvend, nomevend)) 
            cursor.close()
...
def Func_envia_dadoTV_tela(event): # Select da linha da tabela para Tela
    ...
    for selection in tv.selection(): 
        ...
        idVendP, nomeVendP = item["values"][0:2]  
    print("idvendedP= ",idVendP)      
    EnomeVend.delete(0, END) 
    ...
    try:        
        cursor = connect.cursor()
        sql_select_query = """SELECT nomevend \
        FROM Tbvendedor WHERE idvend = %s""" 
        cursor.execute(sql_select_query, (chave_banco,))                    
        rows = cursor.fetchall()
        for col in rows:
            EnomeVend.insert(0, col[0])
          
                    
            return "Carga das colunas da Tabela com sucesso!"
    except Exception as e:
        print('Exception: {}'.format(e))
        return "Erro no Select tab de VENDEDOR"

def Func_cadastrar():
    print('entrou cad_VENDEDOR')

    nomeVendVar = EnomeVend.get()

    if nomeVendVar == "":
        mensagem['text'] = "Nome do vendedor em branco!"
        EnomeVend.focus_set()
        return
    elif len(nomeVendVar) > 60:
        mensagem['text'] = "Nome com mais de 60 caracteres"
        EnomeVend.focus_set()
        return

    try:
        cursor = connect.cursor() 
        QUERY_INSERT = """INSERT INTO tbvendedor(nomevend) VALUES (%s)"""
        RECORD_INSERT = (nomeVendVar)
        cursor.execute(QUERY_INSERT ,(RECORD_INSERT,))
        connect.commit()
        cursor.close()
        mensagem["text"] = "VENDEDOR CADASTRADO"
    except Exception as erro:
        mensagem["text"] = "Erro no Cadastro"
        print('Exception: {}'.format(erro))
        raise Exception(erro)

    EnomeVend.delete(0, END) #limpa os campo
...
def Func_alterar():
    ...
    try:
        cursor = connect.cursor()
        sql_update_query = """UPDATE tbvendedor SET nomevend = %s WHERE idvend = %s"""
        cursor.execute(sql_update_query, (nomeVendVar, chave_banco))           
        connect.commit()
        cursor.close()        
        mensagem["text"] = "VENDEDOR atualizado"
...
def Func_excluir():
    ...
    try:
            cursor = connect.cursor()
            sql_delete_query = """DELETE FROM tbvendedor WHERE idvend = %s"""
            cursor.execute(sql_delete_query, (chave_banco,))
            connect.commit()
            cursor.close()
            mensagem["text"] = "VENDEDOR deletado"
    except psycopg2.Error as erro:
            print("Erro no Delete tab VENDEDOR: ", erro)           
...
tv.heading(1, text="ID VEND")
tv.column("#1",minwidth=0,width=100)
tv.heading(2, text="NOME VEND")
tv.column("#2",minwidth=0,width=500)
